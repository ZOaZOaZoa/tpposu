...
class RegistratorGUI:
    def __init__(self, parent = None, tab_name: str = ''):
        ...
        self.frame_count_entry = ttk.Entry(parent, width=20)
        self.frame_count_entry.pack()
        self.frame_count_entry.insert(0, "100")

        # --- Кнопки ---
        self.start_button = ttk.Button(parent, text="Начать регистрацию", command=self.start)
        self.start_button.pack(pady=5)

        self.stop_button = ttk.Button(parent, text="Остановить", command=self.stop)
        self.stop_button.pack(pady=5)

        # --- ФИО ---
        self.fio_label = ttk.Label(parent, text="ФИО оператора:")
        self.fio_label.pack()

        self.fio_entry = ttk.Entry(parent, width=40)
        self.fio_entry.pack()
        self.fio_entry.insert(0, "Иванов Иван")

        # --- Описание эксперимента ---
        self.description_label = ttk.Label(parent, text="Описание эксперимента:")
        self.description_label.pack()

        self.description_entry = ttk.Entry(parent, width=40)
        self.description_entry.pack()
        self.description_entry.insert(0, "Описания не задано")

        self.save_button = ttk.Button(parent, text="Сохранить в БД", command=self.save)
        self.save_button.pack(pady=5)

        # --- Текущие измерения ---
        self.output_label = ttk.Label(parent, text="Текущий кадр:")
        self.output_label.pack()

        self.output_text = tk.Text(parent, height=30, width=80)
